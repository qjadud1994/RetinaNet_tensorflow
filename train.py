...
#### Train dataset
tf.app.flags.DEFINE_string('train_path', '/root/DB/VOC/VOC2012/tfrecord/',
...
tf.app.flags.DEFINE_integer('valid_steps', 300,
                            """Validation steps""")
...
def _get_init_pretrained():
    ...
    def init_fn(scaffold, sess): return saver_reader.restore(sess, model_path)
    return init_fn
...
def _single_tower(net, tower_indx, input_feature, learning_rate=None, name='train'):
    ...
    with tf.device('/gpu:%d' % tower_indx):
        with tf.variable_scope('{}_tower_{}'.format(name, tower_indx)) as scope:
            ...
            # TODO: Detection output visualize

            if name is 'valid':
                summary_images = []
                for i in range(3):
                    pred_boxes, _, _ = net.decode(logits[0][i], logits[1][i])

                    pred_boxes /= FLAGS.input_size
                    pred_boxes = tf.clip_by_value(pred_boxes, 0.0, 1.0)

                    pred_img = tf.image.draw_bounding_boxes(tf.expand_dims(input_feature.image[i], 0), 
                                                            tf.expand_dims(pred_boxes, 0))
...
def main(argv=None):
    ...
    with tf.Graph().as_default():
        ...
        if FLAGS.use_profile:
            profiler_hook = tf.train.ProfilerHook(save_steps=FLAGS.save_steps,
...
#            hooks.append(cli_debug_hook)
        ...
        with tf.train.MonitoredTrainingSession(checkpoint_dir=FLAGS.output,
            ...
            while _step < FLAGS.max_num_steps:
                ...
                # Print Error (train/valid)
                if FLAGS.use_validation and _step > 0 and \
                    _step % FLAGS.valid_steps == 0:
                    print('STEP : %d\tTRAIN_TOTAL_LOSS : %.8f\tTRAIN_LOC_LOSS : %.8f\tTRAIN_CLS_LOSS : %.5f' 
                          % (_step, step_loc_loss + step_cls_loss, step_loc_loss, step_cls_loss))
                    ...
                    [valid_step_loc_loss, valid_step_cls_loss,  valid_summary] = sess.run([valid_tower_output.loc_loss, 
                                                                                           ...
                                                                                           valid_summary_op])
                    valid_step_loss = valid_step_loc_loss + valid_step_cls_loss
                    if valid_step_loss < best_valid_loss:
                        best_valid_loss = valid_step_loss
